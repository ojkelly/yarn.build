...
export const displayDependency = ({format, circular, dependency, project, padding = 0}: DisplayDependencyProps): void => {
    if (circular) {
    ...
    displayWorkspace({format, project, parent: getName(workspace.manifest.name), padding: padding + 1, workspace});
};
